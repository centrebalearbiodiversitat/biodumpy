...
IUCN_REGIONS = [
    "northern_africa", "global", "pan-africa", 
    "central_africa", "eastern_africa", 
    "northeastern_africa", "western_africa", 
    "southern_africa", "mediterranean", "europe"
]
...
def iucn_query(query, api_key, habitat, regions, historical, threats, output_format):
    ...
    bdp = Biodumpy([IUCN(bulk=True, api_key=api_key, habitat=habitat, regions=regions,
                         historical=historical, threats=threats, output_format=output_format)])
...
def test_download(query, regions, habitat, historical, threats, output_format):
    with redirect_stdout(trap):
        data = iucn_query(
            query=query, regions=regions, habitat=habitat, 
            historical=historical, threats=threats, 
            output_format=output_format, api_key=API_KEY
        )
