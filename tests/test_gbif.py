...
def gbif_query(query, accepted_only, occ, geometry):
    ...
    bdp = Biodumpy([GBIF(output_format='json', dataset_key=gbif_backbone, limit=20,
                         bulk=False, accepted_only=accepted_only, occ=occ, geometry=geometry)])
...
@pytest.mark.parametrize('query, accepted_only, occ, geometry', [
    (['Alytes muletensis (Sanchíz & Adrover, 1979)'], True, False, None),  # accepted_only=True, occ=False, geometry...
    (['Alytes muletensis (Sanchíz & Adrover, 1979)'], True, True, 'POLYGON((0.248 37.604, 6.300 37.604, 6.300 41.472, 0.248 41.472, 0.248 37.604))'),  # accepted_only=True, occ=True, geometry...
])
