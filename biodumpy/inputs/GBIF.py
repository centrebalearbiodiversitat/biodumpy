...
class GBIF(Input):
	...
	def __init__(self, dataset_key='d7dddbf4-2cf0-4f39-9b2a-bb099caae36c', limit=20, accepted_only=True, occ=False, geometry=None,
	             output_format='json', bulk=False):
	...
	def _download(self, query, **kwargs) -> list:
		...
		if response.content:
			...
			if self.accepted:
				...
				payload = list(filter(
					lambda x: x.get('taxonomicStatus') == 'ACCEPTED' and str(query[0]) in x.get('scientificName', ''),
					data))
